...
class ExamService {
  ...
  static async generateResultsPdf(examId, res) {
    ...
    // Load school setting for name/logo
    let schoolName = 'VMS School';
    try {
      const SchoolSetting = require('../../models/SchoolSetting');
      const setting = await SchoolSetting.findOne().lean();
      if (setting?.schoolName) schoolName = setting.schoolName;
    } catch { /* use default */ }
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="results_${(exam.examName || exam.name || 'exam').replace(/\s+/g, '_')}.pdf"`
    );
    doc.pipe(res);
    ...
    doc.fill('#ffffff').fontSize(22).font('Helvetica-Bold')
      .text(schoolName, 40, 20, { width: doc.page.width - 80 });
    doc.fontSize(12).font('Helvetica')
      .text('Exam Result Report', 40, 50);
    ...
    statCols.forEach((s, i) => {
      ...
      doc.fill(GRAY).fontSize(8).font('Helvetica').text(s.label, x + 6, statsY + 6, { width: colW - 12 });
      doc.fill(s.color || DARK).fontSize(11).font('Helvetica-Bold').text(s.value, x + 6, statsY + 22, { width: colW - 12 });
    });
    ...
    headers.forEach((h, i) => {
      doc.fill('#ffffff').fontSize(9).font('Helvetica-Bold').text(h, x + 3, tableTop + 5, { width: colWidths[i] - 6 });
      x += colWidths[i];
    });
    ...
    students.forEach((r, idx) => {
      ...
      cells.forEach((cell, ci) => {
        ...
        doc.fill(color).fontSize(8).font(ci === 7 ? 'Helvetica-Bold' : 'Helvetica')
    ...
    doc.fill(GRAY).fontSize(8).font('Helvetica')
      .text(
        `Generated by ${schoolName} ERP  |  ${new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })}`,
