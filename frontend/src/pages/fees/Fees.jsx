...
import {
  ...
  Card, Statistic, Empty, Tag, Drawer, Descriptions, Divider, Badge,
} from 'antd';
import {
...
const statusColorMap = {
  Paid: 'green', Partial: 'orange', Pending: 'red', unpaid: 'red', partial: 'orange', paid: 'green',
};
...
const Fees = () => {
  ...
  const [statusFilter, setStatusFilter] = useState(undefined); // Paid | Partial | Pending
  ...
  const columns = [
    ...
    { title: 'Class', dataIndex: 'className', key: 'className', width: 110 },
    {
      ...
      render: (_, r) => r.invoiceNumber
        ? <Tag color="blue" style={{ cursor: 'pointer', fontSize: 11 }} onClick={() => openInvoice(r._id, r.name)}>
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Button size="small" icon={<FileTextOutlined />}
            onClick={() => openInvoice(record._id, record.name)}
            id={`view-invoice-${record._id}`}
          >Invoice</Button>
        </Space>
      ),
    },
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <Text type="secondary">Track invoices, collect fees, and monitor dues</Text>
      </div>
        ...
        <Col xs={12} sm={6}>
          <Card size="small" variant="borderless" style={{ borderRadius: 10, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Statistic
              ...
              value={paidCount}
              suffix={<span style={{ fontSize: 13, color: '#94A3B8' }}>/ {filtered.length}</span>}
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
        <Col xs={24} sm={8} md={5}>
          <Select
            placeholder="Filter by class" allowClear style={{ width: '100%' }}
      ...
      <Table
        ...
        rowClassName={(r) => r.totalDue > 0 && r.status !== 'Paid' ? '' : ''}
      ...
      >
        {invoiceData ? (
          <>
            ...
            {invoiceData.feeItems && invoiceData.feeItems.length > 0 && (
              <>
